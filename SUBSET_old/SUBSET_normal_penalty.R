...
SUBSET.normal_penalty_unscaled=function(Y,alpha=2*log(dim(Y)[1])){#Y is the data matrix such that the (i,j)th entry corresponds to the jth entry for the ith variate; alpha is the penalty value incurred for adding another variate to the affected set in the sparse setting.
  n=dim(Y)[2]
  p=dim(Y)[1]
