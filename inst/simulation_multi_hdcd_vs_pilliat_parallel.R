  ...
  for (i in 1:length(ns)) {
    ...
    for(j in 1:length(ps)){
      ...
      for (y in 1:numconfig) {
        ...
        a = proc.time()
        res = Pilliat(X, threshold_d_const=2, threshold_bj_const=6, threshold_partial_const=2,
...
if(save){
  ...
  printlines = c("%%REMEMBER to use package \\usepackage{rotating}!!",
                 ...
                 "$n$ & $p$ & Sparsity & K & \\text{HDCD fast} & \\text{Pilliat} & \\text{HDCD}& \\text{HDCD fast} & \\text{Pilliat} & \\text{HDCD} & \\text{HDCD fast} & \\text{Pilliat} & \\text{HDCD}  & \\text{HDCD fast} & \\text{Pilliat} & \\text{HDCD} \\\\", 
                 "\\hline \\")
  ...
  for (i in 1:length(ns)) {
    ...
    for(j in 1:length(ps)){
      ...
      for (y in 1:numconfig) {
        ...
        res = round(c(hdcd_fast_hausd[i,j,y],pilliat_hausd[i,j,y], hdcd_slow_hausd[i,j,y]),digits=3)
        minind = (res==min(na.omit(res)))
        res = c(hdcd_fast_hausd[i,j,y],pilliat_hausd[i,j,y], hdcd_slow_hausd[i,j,y])
        ...
        res = round(c(hdcd_fast_Kerr[i,j,y],pilliat_Kerr[i,j,y], hdcd_slow_Kerr[i,j,y]),digits=3)
        minind = (abs(res)==min(abs(res)))
        res = c(hdcd_fast_Kerr[i,j,y],pilliat_Kerr[i,j,y], hdcd_slow_Kerr[i,j,y])
        ...
        res = round(c(hdcd_fast_ari[i,j,y],pilliat_ari[i,j,y], hdcd_slow_ari[i,j,y]),digits=3)
        minind = (abs(res)==max(abs(res)))
        res = c(hdcd_fast_ari[i,j,y],pilliat_ari[i,j,y], hdcd_slow_ari[i,j,y])
        for (t in 1:length(res)) {
          if(minind[t]){
            string = sprintf("%s & \\textbf{%.3f} ", string, res[t])
          }else{
        ...
        res = round(1000*c(hdcd_fast_time[i,j,y],pilliat_time[i,j,y], hdcd_slow_time[i,j,y]),digits=3)
        minind = (res==min(res))
        res = 1000*c(hdcd_fast_time[i,j,y],pilliat_time[i,j,y], hdcd_slow_time[i,j,y])
