...
result = foreach(z = 1:N, .combine="cbind") %dopar% {
  rez = c()
  for (i in 1:length(ns)) {
    ...
    for(j in 1:length(ps)){
      ...
      for (y in 1:kvals) {
        ...
        rez = cbind(rez, c(z,i,j,y,k,inspect_res, inspect_time, hdcd_res, hdcd_time,hdcd_s))
...
result_uneven = foreach(z = 1:N, .combine="cbind") %dopar% {
  rez = c()
  for (i in 1:length(ns)) {
    n = ns[i]
    for(j in 1:length(ps)){
