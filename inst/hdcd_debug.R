...
for (j in 1:length(etas)) {
  ...
  if(sparsities[j]==1){
    ...
    #dense
    sparsities[j] = sample(min(p,ceiling(sqrt(p*log(n)))):p,1)
    k = sparsities[j]
...
res = HDCD(X, alpha = 2, K=4,fast = TRUE, rescale_variance = FALSE,debug=TRUE)
sink()
etas
res$changepoints
...
pp = single_HDCD(X[,157:173], threshold_d = 1.5, threshold_s = 1)
pp$pos +156

pp = single_HDCD(X[,41:157], threshold_d = 1.5, threshold_s = 1)
pp$pos +40
# empirical 
calibrated = TRUE
if(!calibrated){
...
sink(file = "/Users/peraugust/Library/CloudStorage/OneDrive-UniversitetetiOslo/project1/simulations/Simulations_HDCD/sinks/sink2.txt")
res5 = HDCD(X, threshold_d = 2, threshold_s = 1, alpha = 1.5, K=5,fast = FALSE,rescale_variance=FALSE,droppartialsum = TRUE,
            thresholds_test = rr2[[1]],debug=TRUE)
sink()
etas
res5$changepoints
sparsities
...
# WORKS WELL WITH THRESHOLD_D = 1.5, THRESHOLD_S = 1.0
# AND CONSTS = 4!
...
for (i in 1:N) {
  ...
  res5 = HDCD(X, alpha = 1.2, K=5,fast = FALSE,rescale_variance=FALSE,droppartialsum = TRUE,
              thresholds = rr2[[1]],thresholds_test = rr2[[1]])
  etas
  res5$changepoints
  sparsities
  ...
  res7 = HDCD(X, threshold_d = 1.5, threshold_s = 1,alpha = 1.2, K=5,fast = FALSE,rescale_variance=FALSE,droppartialsum = TRUE,
              thresholds_test = rr2[[1]])
  etas
  res7$changepoints
  sparsities
  
  rez7 = rez7 + hausdorff(res7$changepoints, etas,n) / N
  kerr7 = kerr7 +abs(length(etas)-length(res7$changepoints))/ N
  ...
  # if(hausdorff(sort(res6[[2]]), etas,n) <  hausdorff(res5$changepoints, etas,n)){
  #   if(length(res6[[2]]) == length(res5$changepoints)){
  #     print("")
  #     print(res5$changepoints)
  #     print(sort(res6[[2]]))
  #     print(etas)
  #     print(sparsities)
  #     print("")
...
for (i in 1:N) {
  ...
  res5 = HDCD(X, thresholds = rr2[[1]],threshold_d = 1.5, threshold_s = 1.0, alpha = 1.2, K=5,fast = FALSE,rescale_variance=FALSE,droppartialsum = TRUE,
            )  
  # res5 = HDCD(X, thresholds = rr2[[1]],threshold_d = 1.5, threshold_s = 1, alpha = 1.2, K=5,fast = FALSE,rescale_variance=FALSE,droppartialsum = TRUE,
  #             thresholds_test = rr2[[1]])
  if(hausdorff(etas, res5$changepoints, n) != 0){
    for (hh in 1:length(res5$changepoints)) {
      kk = res5$changepoints[hh]
      vv = which.min(abs(kk- etas))
      if(min(abs(kk - etas))>0){
        spars = c(spars, res5$s[[hh]])
        tspars = c(tspars, sparsities[vv])
      }
    }
  }
}

hist(tspars, breaks=50)
hist(spars, breaks=50)
