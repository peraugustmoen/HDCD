...
#' @export
changepoint_test_HDCD = function(X, empirical =FALSE, N = 100, tol = 0.05,thresholds=NULL, threshold_d, threshold_s,debug =FALSE,
  ...
  if(empirical){
    if(is.null(thresholds)){
      res = changepoint_test_HDCD_calibrate(n,p, as, nu_as, ts, twologn,N,tol,debug)
      if(droppartialsum){
        thresholds = res[[1]]
      }
      else{
  ...
  res = .Call(cHDCD_test, as.numeric(X), as.integer(n), as.integer(p), as.numeric(thresholds), 
...
#' @export
changepoint_test_HDCD_calibrate = function(n,p, as=NULL, nu_as=NULL, ts=NULL, twologn=NULL, N, tol,debug=FALSE){
  ...
  res= .Call(cHDCD_test_calibrate, as.integer(n), as.integer(p), as.integer(N), 
...
#' @export
changepoint_test_HDCD_modified = function(X, empirical =FALSE, N = 100, tol = 0.05,thresholds=NULL, threshold_d, threshold_s,debug =FALSE,
                                 droppartialsum = FALSE ){
  p = dim(X)[1]
  n = dim(X)[2]
...
#' @export
changepoint_test_HDCD_modified_calibrate = function(n,p, as=NULL, nu_as=NULL, ts=NULL, twologn=NULL, N, tol,debug=FALSE){
  if(is.null(as) || is.null(nu_as) || is.null(ts) || is.null(twologn)){
    max_s = min(sqrt(p*log(n)), p)
    log2ss = 0:floor(log(max_s, base=2))
    ss = 2^(log2ss)
